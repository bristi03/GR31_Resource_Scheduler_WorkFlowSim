...
public class HEFTMLPlanningAlgorithm extends BasePlanningAlgorithm {
        ...
        for (Task task : getTaskList()) {
            ...
            for (Object vmObject : getVmList()) {
                ...
                if (vm.getNumberOfPes() < task.getNumberOfPes()) {
                    ...
                    double predictedTime = getPredictedExecutionTime(task, vm);
                    double calculatedTime = task.getCloudletTotalLength() / vm.getMips();

                    double hybridExecutionTime = (0.6 * predictedTime) + (0.4 * calculatedTime);

                    costsVm.put(vm, hybridExecutionTime);

                    // costsVm.put(vm, predictedTime);
                    // costsVm.put(vm,
    ...
    private double getPredictedExecutionTime(Task task, CondorVM vm) {
        try {
            ...
            json.put("TaskMI", task.getCloudletTotalLength());
            json.put("TaskSize", task.getCloudletFileSize());
            json.put("Job_Depth", task.getDepth());
            json.put("MIPs", vm.getMips());
            json.put("Bandwidth", vm.getBw());
            json.put("VM_Memory", vm.getRam());
            json.put("PES", vm.getNumberOfPes());
            json.put("TaskMI_per_MIPS", task.getCloudletTotalLength() / vm.getMips());
            ...
            return result.getDouble("predicted_time");
